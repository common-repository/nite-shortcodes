...
if ( !current_user_can('edit_pages') && !current_user_can('edit_posts') ) 
	wp_die(__("You are not allowed to be here", "niteshortcode"));
    global $wpdb;
?>
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<title>Nite Shortcodes</title>
...
<!-- <form onsubmit="insertLink();return false;" action="#"> -->
	...
	<div>
		...
		<table border="0" cellpadding="4" cellspacing="0">
         <tr>
            <td><label for="mnmshortcode_tag"><?php _e("Select Shortcode", 'niteshortcode'); ?></label></td>
            <td><select id="mnmshortcode_tag" name="mnmshortcode_tag">
                <option value="0">Choose Shortcode</option>
				<?php
					if(is_array($shortcode_tags)) 
					{
						...
						{
							if( stristr($short_code_value, 'nitethemes_') ) 
							{
								$mnm_shortcode_name = str_replace('nitethemes_', '' ,$short_code_value);
	...
	<div class="mceActionPanel">
		<div style="float: left">
			<input type="button" id="cancel" name="cancel" value="<?php _e('Cancel', 'niteshortcode'); ?>" onClick="tinyMCEPopup.close();" />
		</div>

		<div style="float: right">
			<input type="submit" id="insert" name="insert" value="<?php _e('Insert', 'niteshortcode'); ?>" onClick="mnmshortcodesubmit();" />
		</div>
	</div>
</form>
</body>
